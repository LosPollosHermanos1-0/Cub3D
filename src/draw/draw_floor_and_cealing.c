...
uint32_t blend_color(uint32_t originalColor, float blendFactor) {
    ...
    g *= (1 - blendFactor);
    b *= (1 - blendFactor);
    return ((u_int32_t)r << 24) | ((u_int32_t)g << 16) | ((u_int32_t)b << 8) | 0xFF; // Assuming alpha is always 255
}
...
uint32_t blend_two_colors(uint32_t color1, uint32_t color2, float blendFactor) {
    ...
    return ((u_int32_t)rResult << 24) | ((u_int32_t)gResult << 16) | ((u_int32_t)bResult << 8) | 0xFF; // Assuming alpha is always 255
}
...
inline void draw_floor_and_ceiling(t_data *data, int y) {
    ...
    for (int x = 0; x < data->window->width; ++x) {
        ...
        if (is_in_circle((t_vector_2d){20,12}, 0.5, (t_vector_2d){floor.x, floor.y})) {
            floorColor = blend_two_colors(floorColor, ((u_int32_t)255 << 24) | ((u_int32_t)255 << 16) | ((u_int32_t)0 << 8) | 0xFF, 0.1);
        }
