...
/*   Updated: 2024/07/23 19:18:45 by jmoritz          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	key_handler(const mlx_key_data_t keydata, void *param)
{
	...
	if (keydata.key == MLX_KEY_F && keydata.action == MLX_PRESS)
	{
		...
		{
			data->map->map[(int)data->last_faced_open_door.x]
			[(int)data->last_faced_open_door.y] = DOOR_CLOSED;
		}
		else if (data->flags & FLAG_FACING_CLOSED_DOOR)
		{
			data->map->map[(int)data->last_faced_closed_door.x]
			[(int)data->last_faced_closed_door.y] = DOOR_OPEN;
		}
	}
}
